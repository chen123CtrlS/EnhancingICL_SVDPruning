...
# Space before label(xxx) is important otherwise we will get the wrong token_id
# label : world, sports, business, science
token_ids1 = tokenizer(" world")
assert len(token_ids1["input_ids"]) == 1
true_token_id1 = int(token_ids1["input_ids"][0])

token_ids2 = tokenizer(" sports")
assert len(token_ids2["input_ids"]) == 1
true_token_id2 = int(token_ids2["input_ids"][0])

token_ids3 = tokenizer(" business")
assert len(token_ids3["input_ids"]) == 1
true_token_id3 = int(token_ids3["input_ids"][0])

token_ids4 = tokenizer(" science")
assert len(token_ids4["input_ids"]) == 1
true_token_id4 = int(token_ids4["input_ids"][0])
