...
from my_utils.data_process_cl import sample_data_cb,cat_demo_cb
...
# process data
demo_sample,eval_sample,_ = sample_data_cb(dataset['train'],seed=66, actual_sample_size = eval_data_size, demo_size = demo_size)
eval_data = cat_demo_cb(demo_sample,eval_sample,task_name, demo_shot=demo_size)
test_data = cat_demo_cb(demo_sample,dataset['test'],task_name, demo_shot=demo_size)
...
m_token_ids = tokenizer("Maybe")
assert len(m_token_ids["input_ids"]) == 2 and m_token_ids["input_ids"][0] == 1
m_token_id = int(m_token_ids["input_ids"][1])
...
def test(model,test_data):
    ...
    for i,item in enumerate(tqdm(test_data)):
        ...
        if few_shot:
            ...
            demo = ''
        prompt = f"Suppose {premise} Can we infer that '{hypothesis}'? Yes, or No, or Mi? "
        ...
        with torch.no_grad():
            ...
            m_logprob = last_token_logprob[m_token_id].item()
            if label == 0:     # Answer is Yes
                answer_log_prob = true_logprob
                is_correct = true_logprob > false_logprob and true_logprob > m_logprob
                answer = "Yes"
            elif label==1:               # Answer is No
                answer_log_prob = false_logprob
                is_correct = true_logprob < false_logprob and m_logprob < false_logprob
                answer = "No"
            else:
                answer_log_prob = m_logprob
                is_correct = true_logprob < m_logprob and m_logprob > false_logprob
                answer = "Mi"
            if is_correct:
                acc+=1
